...
const themes = {
  ...
  matrix: ['#00ff00', '#00cc00'], // Green theme for UMG matrix
  ocean: ['#00c6ff', '#0072ff'],
...
// UMG-themed matrix effect
const umgMatrixEffect = () => {
  const chars = 'UMGUMGUMG0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  let matrixText = '';
...
const main = async () => {
  try {
    ...
    const themePrompt = createGradientText('Choose a theme (default, matrix, ocean, sunset): ', themes.default);
    ...
    setInterval(() => {
      ...
      // Display UMG matrix effect
      if (theme === 'matrix') {
        console.log(umgMatrixEffect());
      }
